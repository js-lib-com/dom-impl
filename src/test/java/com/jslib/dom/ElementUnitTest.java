...
public class ElementUnitTest extends TestCase
{
  ...
  public void testInsertBefore_ImportNS() throws IOException, SAXException, XPathException
  {
    ...
    Document doc1 = builder().parseXMLNS(xml1);
    Document doc2 = builder().parseXMLNS(xml2);

    Element editable = doc1.getByXPathNS(ns, "//*[@wood:editable]");
    editable.insertBefore(doc2.getRoot());
    editable.remove();

    EList elist = doc1.findByXPathNS(ns, "//*[@wood:template]");
    assertEquals(elist.size(), 1);

    editable = elist.item(0);
    editable.removeAttrNS("js-lib.com/wood", "template");
  }
